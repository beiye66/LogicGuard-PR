...
> **一句话**：开发者提交 PR → 本工具自动抓取代码变更、用大模型审查、把「变更总结 + 风险清单」回帖到 PR 评论区。全自动、零人工。

### 🔍 全面的代码排查能力（聚焦真正的高风险，不堆砌琐碎）

| 类别 | 排查内容 |
|---|---|
| 🧵 **并发安全** | 竞态条件、未加锁的共享变量、原子性问题 |
| 💧 **内存 / 资源** | 内存泄漏、资源未释放、越界访问 |
| 🧠 **逻辑错误** | 错误分支、API 误用、空值 / 空集合处理 |
| 🛡️ **边界情况** | 除零、空输入、未校验的参数 |

- 📍 每条隐患**定位到文件 + 行号**
- 🤫 **低误报**：没有严重问题就直说"未发现明显的高风险问题"，**绝不为凑字数捏造琐碎建议**

### 🚀 创新点（与众不同之处）

1. 🧩 **多模型路由** —— 一套引擎兼容 OpenAI / DeepSeek / Gemini / 豆包（OpenAI 兼容）+ Claude（Anthropic），**配置即切换**
2. 🔑 **BYOK 自带 Key** —— Web 端可填自己的 API Key，用自己的额度审查
3. ♻️ **增量审查** —— PR 多次推送时只审**新增改动**，省 token、不重复唠叨
4. 🌐 **三种形态，一套引擎** —— ① 零配置 **Web 体验端** ② 一行接入的**可复用 GitHub Action** ③ 本地 CLI
5. 🛡️ **工程化扎实** —— 模块化设计、防御性编程、**50 个单元测试 + CI 守护**
