...
- 对每条确实存在的风险，**必须**包含以下三项，缺一不可：
  1. **位置**：涉及的文件名与**具体行号**。行号请依据 diff 的 hunk 头（形如 `@@ -旧起,旧行数 +新起,新行数 @@`）推断：新增/修改行从「新起」开始计数；若跨多行，给出行号范围；并可附上最相关的一行代码片段。
  2. **问题**：该隐患具体是什么。
  3. **后果**：可能造成的影响。
- 行号是硬性要求：无论使用哪种大模型，每条风险都必须给出文件名 + 行号（例如「`account_service.py` 第 12 行」），不得只写文件名而省略行号。
"""
